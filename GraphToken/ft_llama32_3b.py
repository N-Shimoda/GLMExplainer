...
# -*- coding: utf-8 -*-
"""
Llama 3.2-3B (Instruct) を GraphQA で QLoRA 微調整するスクリプト
- モデル: meta-llama/Llama-3.2-3B-Instruct（デフォルト、変更可）
- データ: baharef/GraphQA（subset と split を引数で選択）
- 方式: TRL SFTTrainer + PEFT(LoRA) + bitsandbytes 4bit (QLoRA)
- 評価: 簡易 Exact Match（空白・改行・末尾ピリオドを無視）
"""
...
def build_args():
    ...
    p.add_argument(
        ...
        help="ベースモデル（例: meta-llama/Llama-3.2-3B[-Instruct]）",
    )
    p.add_argument(
        "--subset",
        type=str,
        default="connected_nodes",
        help="GraphQA subset（connected_nodes, disconnected_nodes, ...）",
    )
    ...
    p.add_argument("--lr", type=float, default=1e-4)  # LoRA なのでやや大きめ
...
def build_prompt(question: str) -> str:
    # 不要な空白を削るだけ（GraphQAのquestionは整形済み）
...
def main():
    ...
    # TRL が解釈できる会話型 prompt-completion へ変換
    ...
    sft_cfg = SFTConfig(
        ...
        packing=True,  # サンプルを連結して効率化
        bf16=True,
        optim="adamw_8bit",  # 8bit Optim (bitsandbytes)
        report_to="none",
        completion_only_loss=True,  # prompt 部分は loss から除外
        model_init_kwargs={
            "quantization_config": bnb_config,
            "device_map": "auto",
            "trust_remote_code": True,
    ...
    # 学習
    trainer.train()
    trainer.save_model()  # LoRA アダプタを含む PeftModel と tokenizer を保存
